...
int cur_left_vel,cur_right_vel,left_vel,right_vel;
int linear_vel,angular_vel=0;
double L=0.42;
int bm[12]={0};
double xm[6]={0};
...
void transfer2trackvel(double linear_vel, double angular_vel)
{
		if(linear_vel==0 && angular_vel!=0){    //原地旋轉
			if(angular_vel>0){
				left_vel=(L/2)*angular_vel;
			  right_vel=(L/2)*angular_vel;
				state[0]=false;
				state[1]=true;
				state[2]=false;
				state[3]=false;
			...
			{
				left_vel=(L/2)*abs(angular_vel);
			  right_vel=(L/2)*abs(angular_vel);
				state[0]=false;
				state[1]=false;
				state[2]=false;
				state[3]=true;
		...
		if(linear_vel!=0 && angular_vel==0){     //直線前進或後退 
			if(linear_vel>0){
				left_vel=linear_vel;
			...
			{
				left_vel=abs(linear_vel);
				right_vel=abs(linear_vel);
				state[0]=false;
				state[1]=true;
				state[2]=false;
				state[3]=true;
		...
		if(linear_vel>0 && angular_vel!=0){      //向前左彎或向右彎 
			if(linear_vel>0 && angular_vel>0){
				right_vel=linear_vel+(L/2)*angular_vel;
				left_vel=linear_vel-(L/2)*angular_vel;
				state[0]=false;
				state[1]=false;
				state[2]=false;
				state[3]=false;
			...
			if (right_vel<0){
					right_vel=abs(right_vel);
					state[3]=true;
			}
			if (left_vel<0){
					left_vel=abs(left_vel);
				  state[1]=true;
			}
		}
		...
		if(linear_vel<0 && angular_vel!=0){       //向後左彎或向右彎
			if(linear_vel<0 && angular_vel>0){
				right_vel=abs(linear_vel)+(L/2)*angular_vel;
				left_vel=abs(linear_vel)-(L/2)*angular_vel;
				state[0]=false;
				state[1]=true;
				state[2]=false;
				state[3]=true;
		...
		if(linear_vel==0 && angular_vel==0){       //Stop
			state[0]=true;
			state[2]=true;
		}
		...
		if(xm[4]==1)	
			angular_vel = angular_vel + 100;
		else if (xm[4]==-1)
			angular_vel = angular_vel - 100;	
		if(xm[5]==1)	//上下鍵
			linear_vel = linear_vel + 100;
		else if (xm[5]==-1)
			linear_vel = linear_vel - 100;

}
/*
state[0]=true;  	state[2]=true;   //5v disable for EN  
state[0]=false;     //0v enable the EN
state[1]=false;    state[3]=false //0v enable S/R enable positively rotate for left track
state[1]=true;    state[3]=true;  //5v enable S/R enable negatively rotate for left track   */
