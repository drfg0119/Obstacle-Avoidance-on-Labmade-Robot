...
int overrounding_r =0;
int overrounding_l =0;
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        cmd_msg.data.clear();
        overrounding_r = rounding(vr_value * 1853 + 15, 1);
        overrounding_l = rounding(vl_value * 1853 + 15, 1);
        if (state_data == 0)
        { ////0正走 1停止 2反走
	    ...
	    std::cout << "vr right track velocity: " << overrounding_r << std::endl;
            std::cout << "vl left track velocity: " << overrounding_l << std::endl;
